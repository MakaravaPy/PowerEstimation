# -*- coding: utf-8 -*-
"""
Created on Tue Aug 06 14:40:01 2019
...
def JointIteratedKalmanFilter(y,Sm,Yws,Vs,iterstop=50,accuracy=1e-9):
    ...
    x_est=np.zeros((n_up,t_f+1))
    w_est= -0.01*np.ones((n_up,t_f+1))        
        
    mu = np.hstack((v0[:n_K]*np.cos(v0[n_K:]),v0[:n_K]*np.sin(v0[n_K:])))
    
    #kalman filter paras for weight estimation
    Pw = p0*np.eye(n_up)#shape (n_up,n_up); conditional mean covariance matrix
    Qw = 1e-4*np.eye(n_up)#covariance matrix of weight equation; n_up<=22

    for k in range(1,t_f+1):
        
        wf = w_est[:,k-1]
        xf = wf*Sigmoid(x_est[:,k-1])
        
        F = transition(x_est[:,k-1], wf)
...
#        while(est_error3 > accuracy) and (counter3 <7):
        while(est_error3 > accuracy) and (counter3 < 10):
            
            Dh = jacobian(Y00,Ys,mu,Vs[:,k-1])
            H = np.dot(Cm,np.dot(np.linalg.inv(Dh),Dnm))
            Hw = weight_jacobian(H, xf)#shape(len(vol), n_up)
        ...
        w_est[:,k]= eta_w
        Pw = np.dot(np.eye(n_up)-np.dot(Kw,Hw),Pwf)
...
print mse
xx_est = np.loadtxt("AllS_est_no_online.out")

xxx_est = np.loadtxt("S_est_no_online.out")
xx6 = np.loadtxt("IterS6.out")



figure(44); clf()    # active power
for i,k in enumerate([1,3,6,10]):
#for k in range(n_K):
    subplot(2,2,i+1)
    plot(time,-S[k+n_K,:],'b-D',linewidth=2.5,label="True Value", ms=5)
    plot(time,-S_est[k+n_K,:],'r-o',linewidth=3.5,label="Joint 15", ms=5)
...
plot(time,-S[8+n_K,:],'b-D',linewidth=2.5,label="true value", ms=5)
plot(time,-S_est[8+n_K,:],'r-o',linewidth=3.5,label="estimated online", ms=5)
plot(time,-S_est[8+n_K,:],'m-o',linewidth=3.5,label="Joint 15", ms=5)
plot(time,-Sfc[8+n_K,:],'c-',linewidth=2.5,label="pseudo-measurements") 
#plot(time,xx_est[k,:],'m-*',linewidth=3.5,label="estimated fixed",ms=5)
legend(loc="upper right")
title("node 9", fontweight='bold')
